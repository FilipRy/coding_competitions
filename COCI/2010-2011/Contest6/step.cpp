    ...
    for(int i=0;i<Q;i++){
        ...
        for(int k=0;k<m.size();k++){
            if(x>=m[k].first&&m[k].first+m[k].second/10>x&&m[k].second%10==0){
                if(y==-10){
                    if(m[k].first+m[k].second/10-x>2&&x-1-m[k].first>0){
                        m.resize(m.size()+1);
                        y=m.size()-1;}
                    else if(m[k].first+m[k].second/10-x>2){
                        y=k;
                    }
            ...
            if(m[k].first+m[k].second/10==x&&m[k].second%10==0&&ch[m[k].first+m[k].second/10-1]==c){
                if(f){
            ...
            //cout<<" k "<<k<<" m.f "<<m[k].first<<" ch "<<ch[m[k].first-1]<<'\n';
            if(m[k].first-1==x&&m[k].second%10==0&&ch[m[k].first]==c){
                if(f){
